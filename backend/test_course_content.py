...
class CourseContentTest(unittest.TestCase):
    def test_courses_have_matching_unique_missions(self) -> None:
        self.assertEqual(len(ACTIVE_MODULE_SLUGS), 27)
        self.assertEqual(len(set(ACTIVE_MODULE_SLUGS)), 27)
        ...
        self.assertEqual([module["order_index"] for module in KAZAKH_MODULES], list(range(1, 10)))
        self.assertEqual(
        ...
        self.assertEqual(
            COURSE_MODULE_SLUGS["en"],
            tuple(base_module_slug(slug) for slug in COURSE_MODULE_SLUGS["kk"]),
        )
        self.assertTrue(all(course_id_for_slug(slug) == "kk" for slug in COURSE_MODULE_SLUGS["kk"]))
